...
class Tello(Action): #Action클래스를 상속받는 Tello 객체.
    def __init__(self, name : str, tello_address : str, port : int, pipe : Any, drone_location_array : SynchronizedArray, tello_location_array : SynchronizedArray) -> None:
        ...
        try:
            ...
            self.socket_tello.bind(("0.0.0.0",self.port))    # Tello 명령 수신 소켓 바인딩
    ...
    def get_drone_location(self) -> List[float]:    #드론의 위치를 반환하는 함수
        with self.drone_location_array.get_lock():    #드론 위치를 저장할 배열에 대한 락을 걸어줌.
            drone_location = list(self.drone_location_array) #드론 위치를 저장할 배열을 복사함.
        return drone_location #드론 위치를 반환함.
    
    
    def get_tello_location(self) -> List[float]:
    ...
    def tello_control(self): #이 함수는 commander 객체(메인프로세스)에서 파이프로부터 받아온 메세지를 실행하는 부분임.(계속해서 명령을 수행할 부분) 
        ...
        while True:
            ...
            func_name, args, kwargs = self.tello_to_main_pipe.recv()
            try:
                func = getattr(self, func_name) #self(tello 객체)에서 func_name의 동작을 하도록 함. ex)tello.forward(), tello.takeoff()
                func(*args, **kwargs) #함수를 실행할 때 인자를 받는 부분. 
                #print(f"[{self.tello_address}] {func_name} 실행됨")
